...
def main():
    ...
    hosts = int(job_properties['resources'].get('hosts', '1'))
    ...
    name = job_properties['cluster'].get('name', 'hodgkins').replace("=", "_")
    sla = job_properties['cluster'].get('sla', 'DEVEL').replace("=", "_")

    # Submit the job to the queue.
    run_bsub(name, queue, threads, mem, hosts, runtime, sla, out, err, args.jobscript)

def run_bsub(name, queue, threads, mem, hosts, runtime, sla, stdout, stderr, script):
