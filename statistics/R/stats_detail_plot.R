...
conditionsToPlot <- function(statsRes, calcColumn = "Fraction", statsFilter = NULL, orderBy = NULL, maxConds = NULL){
    medCols <- names(statsRes)[grepl(paste0("median ", tolower(calcColumn)), names(statsRes))]
    fdrCol  <- paste(calcColumn, "adjusted p.value")
    if(!fdrCol %in% names(statsRes)){ fdrCol <- "adjusted p.value" }
    ...
    if(!is.null(maxConds)){
        ids <- ids[1:min(maxConds, length(ids))]
    }
    ids <- statsRes %>% filter(`Cell State ID` %in% ids) %>% arrange_at(orderBy) %>% pull(`Cell State ID`)
    ids
}
...
#' @param maxConds         maximum number of conditions to plot; default = 30
#' 
#' @return list with three elements: labels, fovVals, and stats
getStatsDetailData <- function(statsFile, sampAnn, question, conds, 
...
log_debug("sheetName = ", sheetName)                                            
    ...
    idOrder <- rev(c2p)   ### TODO: FIGURE OUT BETTER WAY TO USE ALL OF THESE ORDER OPTIONS
    sGrps <- getSampleGroups(question, sampAnn)

    log_debug("Formatting conditions for plotting")
...
print(labs)
    ...
    log_debug("Formatting stats for plotting")
...
print(stats)
    log_debug(paste0("Formatting ", calcUnit, " level data for plotting"))
log_debug("statsFile = ", statsFile)
log_debug("sheetName = ", sheetName)
    fovDat <- formatFOVdataForPlottingDetail(question, sGrps, c2p, conds, 
...
getCellTypeLabel <- function(ct, cellTypes, subtype = NULL){
    ...
    if(grepl("_All$", ct)){
        label <- gsub("_All", "", ct)
        subs <- "All"
        return(bquote(.(label)[.(subs)]))
    } else if(ct == "HRS"){
        return(ct)
    }
    ##########################

    idCols <- c("Category", "Cell_type", "Subtype")
    ctInfo <- cellTypes %>%
              select_at(c(idCols, "Cell_type_figures", "Cell_type_figures_subscript", 
                           "Subtype_figures", "Subtype_figures_subscript")) %>%
              filter_at(all_of(idCols), any_vars(. %in% c(ct, subtype))) %>%
              unique()
#print(ct)
    if(nrow(ctInfo) == 0){ 
        return(ct)
        ...
        #}
    #}
    labelCols <- list(Category = c("Category",""),
                      Cell_type = c("Cell_type_figures", "Cell_type_figures_subscript"),
                      Subtype = c("Subtype_figures", "Subtype_figures_subscript"))

    labelCols <- labelCols[[names(ctInfo)[ctInfo == ct][1]]] # Leukocyte_Other is the same for cell type and subtype so need the 1 index here just to choose one of them
   #print(labelCols) 
    label <- ctInfo[[labelCols[1]]]
    subs <- ctInfo[[labelCols[2]]]
#print(paste0("  ", subs))
    if(!is.na(subs)){
        label <- gsub(subs, "", label)
    }
...
conditionLabel <- function(labelStr, cellTypes, delim = " | ", returnExpression = TRUE){
#print(paste0("labelStr: ", labelStr))
...
addFacetSpacerRows <- function(dat, facetY, xCol = "Column", yCol = "y", bgColor = "white"){
    ...
    if(all(lapply(facetY, function(x){ length(unique(dat[[x]])) == 1 }))){ return(dat) }
...
#' @param panelOrder      character vector indicating the order in which the three panels, 'labels',
#'                        'distribution' (fraction or density values and medians) and 'stats' 
#'                        (OR or FC), should appear; default: c('labels', 'distribution', 'stats')
#' 
...
plotQuestionResults <- function(dat, stats, conditions, clrs, groupVar, calcType = "fractions",
                                ...
                                yNudge=0.1, facetY = NULL, fontsize = 8, spacerColor = "#D8D8D8",
                                stripeBG = TRUE, bgColor = "white", stripeColor = "#f0f0f0",
                                keepLegend = T, stripWidth = unit(2,"cm"), separateFacets = TRUE,
                                rel_widths = c(1, 0.7, 0.6), violin = FALSE,
                                panelOrder = c("labels", "distribution", "stats")){
    ...
    detPlot   <- plotQuestionDetail(dat, groupVar, clrs, xVar = xVar, yVar = yVar, yNudge = yNudge,
                                    ...
                                    separateFacets = separateFacets, violin = violin)

    statPlot  <- plotEffectSize(stats, clrs = NULL, 
                                effectCol = ifelse(xVar %in% c("Fraction","MeanFraction"), 
                                                   "Odds Ratio", "Fold Change"),
                                yVar = yVar, facetY = facetY, separateFacets = separateFacets, 
                                stripeBG = stripeBG, bgColor = bgColor, stripeColor = stripeColor,
                                fontsize = fontsize, spacerColor = spacerColor,
                                keepLegend = keepLegend)

    condDet <- matchPanelHeights(condTbl, detPlot)
    condStat <- matchPanelHeights(condTbl, statPlot)

    lst <- list("labels" = condDet[[1]], "distribution" = condDet[[2]], "stats" = condStat[[2]])

    plot_grid(plotlist = lst[panelOrder], align = 'h', axis = 'bt', nrow = 1, rel_widths = rel_widths)

#plot_grid(condTbl, detPlot, statPlot, align = 'h', axis = 'bt', nrow = 1, rel_widths = rel_widths)
...
#' @param hideIDs         do NOT include cell state IDs in panel
#' @param separateFacets  logical; when TRUE, spacer rows between y facets will be added. 
#'                        default: TRUE
#' 
...
conditionTable <- function(dat, cellTypes, facetY = NULL, stripeBG = TRUE, bgColor = "white",
                           ...
                           stripWidth = unit(2,"cm"), hideIDs = FALSE, separateFacets = TRUE){
print("ct")
    ...
    title <- ifelse(calcType == "fractions", "Subpopulation | Population", "Population")
    xLbls <- c(title, "ID")                

    lbls <- ggplot(tmp, aes(x = x, y = y, hjust = hjust)) + #, label = Value)) +
            geom_rect(aes(xmin = -Inf, xmax = Inf, ymin = ystart, ymax=yend, fill = col), size = 0.25) +
            #geom_text(vjust = 0.5, aes(label = conditionLabel(Value, cellTypes))) + #       ((Value), cellTypes)) +
            scale_y_discrete(expand = c(0, 0)) +
            scale_x_continuous(expand = c(0.01, 0.01), breaks = c(0, 1), labels = xLbls) +
            ...
            theme(panel.background = element_rect(color = "black", fill = "transparent"),
                  axis.text.x = element_text(size = fontsize, hjust = 0),
                  axis.ticks.x = element_blank(),
                  axis.title.x = element_blank(),
#plot.background = element_rect(fill = "pink"),
    plot.margin = margin(l = 1, r = 0, t=3, b=0.5, unit = "cm"))

    if(!is.null(facetY)){
        lbls <- lbls +
                facet_grid(as.formula(paste(paste(facetY, collapse = " + "), "~ .")), 
...
plotQuestionDetail <- function(points, groupVar, clrs, xVar = "Fraction", yVar = "Condition",
                               facetY = NULL, yNudge = 0.1, fontsize = 8,
                               stripeBG=T, bgColor = "white", stripeColor = "#f0f0f0",
                               spacerColor = "#D8D8D8", keepLegend = T, separateFacets = FALSE,
                               violin = FALSE){
print("pqd")
    if(!stripeBG){ stripeColor <- bgColor }
    ...
    points <- points %>%
              formatForStripedBackground(colors = c(bgColor, stripeColor), yVar = "y") %>%
              mutate(y2 = as.numeric(!!as.name(yVar)),
                     smryY = ifelse(!!as.name(groupVar) == levels(points[[groupVar]])[2],
                                    y2 + yNudge, y2 - yNudge))
...
#                   filter_at(vars(facetY[1]), all_vars(!is.na(.)))
        if(separateFacets){
            points <- points %>%
                      addFacetSpacerRows(facetY, xCol = NULL, yCol = "y",
                                         bgColor = spacerColor)
        }
    }

    ### prep colors and annotation
    bgColors <- unique(c(bgColor, stripeColor, spacerColor))
    names(bgColors) <- bgColors
    gridColor       <- "lightgray" #"white"
    gridSize        <- 0.1 #1

    nr <- length(unique(points$`Cell State ID`))
    minLY <- ifelse(xVar == "Density", 1.25, 1.1)
    legendY <- min(minLY, 1 + (0.1 * (60/nr)))
    ...
    ## init plot & draw background
    p <- ggplot(points,
                aes_string(x = xVar, y = yVar, color = groupVar, fill = groupVar)) +
         #geom_rect(aes(xmin = xStripeMin, xmax = Inf, ymin = ystart, ymax = yend, fill = col, col = col),
    ...
    ## actual data
    if(violin){
        ## trim violins to the actual ranges of values for each group+condition
        p <- p +   
geom_density_ridges(color = "black", alpha = 0.5)
       #      geom_violin(trim = TRUE, orientation = "y", width = 0.75, scale = "width", 
       #                  color = "black", position = position_dodge(0.5)) +#, draw_quantiles = 0.5) 
#                 geom_point(aes(x = Median, y = smryY), color = "black", size = 2)
    } else {
        p <- p +
    ...
    p <- p +
         ...
         theme(legend.title = element_blank(),
               ...
               panel.background = element_rect(color = "black", fill = "transparent", size = 0.75),
       #        panel.spacing = unit(0,"lines"),
               strip.background = element_blank(),
               strip.text = element_blank(),
               plot.margin = margin(l = 1, r = 1, t=3, b=0.5, unit = "cm")) +
    ...
    if(!is.null(facetY)){
        ...
        if(separateFacets && any(!is.na(points$labelY) && points$labelY == 0)){
            p <- p +
...
plotEffectSize <- function(dat, effectCol="Odds Ratio", yVar="Condition",
                           ...
                           separateFacets = TRUE, build_plot = TRUE,
                           pMargin = margin(l = 0, r = 1, t=3, b=0.5, unit = "cm")){
print("pes")
    if(!stripeBG){ stripeColor <- bgColor }
    ...
    ## init plot
    p <- ggplot(resTbl, aes(x = !!as.name(effectCol), y = !!as.name(yVar))) +
         ...
         geom_vline(xintercept = 1, size=0.25, linetype = "dashed") +
         ...
         scale_fill_manual(values = allClrs, guide = "none") +
         scale_x_continuous(trans = "log", oob = scales::squish, breaks=breaks, labels=labels, 
                            limits=c(mn,mx), expand = c(0.1, 0.1), sec.axis = dup_axis()) +
         ylab("") +
    ...
    if(colorBy == "signif"){
         p <- p +
              ...
              color = guide_legend("FDR"))
    }

    ## shade space between facets
    if(separateFacets && any(!is.na(resTbl$labelY) && resTbl$labelY == 0)){
        p <- p +
    ...
    if(!is.null(facetY)){
        p <- p +
             facet_grid(as.formula(paste(paste0(facetY,collapse="+"), " ~ .")), scales = "free_y", space="free_y") +
    ...
    if(hideXaxisTop){ 
        p <- p + 
             theme(axis.text.x.top = element_blank(), 
                   ...
                   plot.margin = margin(l = -0.75, r = -0.2, t = 0.5, b = 0.5, unit = "cm"))
    }
...
plotComparisonHeatmap <- function(compDat, compCol, yVar = "y", facetY = NULL, fontsize = 12, 
                                  ...
                                  spacerColor = "#d8d8d8", keepLegend = T, fill = "Value", colorBarLimit = 5,
                                  hideXaxisTop = FALSE, hideXaxisBottom = FALSE, hideXtitle = FALSE, 
                                  gridColor = "darkgray"){
    ...
    breaks <- function(x){
        print(seq(min(x, na.rm = T), max(x, na.rm = T)))
        return(seq(min(x, na.rm = T), max(x, na.rm = T)))
    }
    xLabels <- function(y){
        lbls <- dat %>%
                select(c("x", compCol)) %>%
                filter(x %in% ceiling(y), !is.na(!!as.name(compCol))) %>%
                unique %>% arrange(x) %>% pull(compCol)
        #print(lbls)
        lbls
    }
    ...
    legendY <- 1.2 + (0.1 * (35/nr))

    p <- ggplot(dat %>% filter(!is.na(!!as.name(compCol))), 
                aes_string(x = "x", y = "y2")) + 
         geom_tile(data = dat %>% filter(is.na(!!as.name(fill))), fill = "#E0E0E0", color = gridColor, 
                   height = tileH, width = tileW) 

    if(is.numeric(dat[[fill]])){
        lim <- colorBarLimit
        dat[[fill]][dat[[fill]] > lim] <- lim
        dat[[fill]][dat[[fill]] < -lim] <- -lim

        clrs <- c('CR vs UT' = '#32cd32', 'non-CR vs UT' = '#f5d000',
        ...
        p <- p +
             geom_tile(data = dat, # %>% filter(!is.na(!!as.name(fill))), 
                       ...
                       height = tileH, width = tileW) +
             geom_point(data = dat %>% filter(signif == "*"),
                         aes(shape = signif), size = 1.5) + #, color = signifColor), size = 1.5) +
             scale_color_manual(values = clrs, guide = "none") + 
             scale_fill_gradient2(name = "", guide = "colorbar", 
                                  low = "#104E8B", mid = "#F8F8F8", high = "#b1182a", 
    ...
    yBreaks <- as.numeric(levels(dat$y))
    yLabels <- rev(unique(dat$`Cell State ID`))
    p <- p +
         geom_vline(xintercept = Inf, size = 0.5, color = "black") +
         geom_vline(xintercept = -Inf, size = 0.5, color = "black") +
         geom_hline(yintercept = Inf, size = 0.5, color = "black") +
         geom_hline(yintercept = -Inf, size = 0.5, color = "black") +
         xlab(compCol) +
         scale_y_continuous(expand = c(0,0), breaks = yBreaks, sec.axis = dup_axis()) + #, labels = yLabels, sec.axis = dup_axis()) +
         #scale_y_discrete(expand = c(0,0), breaks = unique(dat$y2), limits = c(min(dat$y2), max(dat$y2))) + 
         scale_x_continuous(breaks = breaks, #limits = c(min(x, na.rm = T) - 0.5, max(x, na.rm = T) + 0.50),
                            expand = c(0,0), sec.axis = dup_axis(), labels = xLabels) +
         ...
         theme(strip.background = element_blank(),
               ...
               plot.margin = margin(l = 0.5, r = -0.8, t=3, b=0.5, unit = "cm"),
               #plot.margin = margin(l = -0.5, r = -1.1, t=3, b=0.5, unit = "cm"),
               #plot.margin = margin(l = -0.2, r = -1.1, t=3, b=0.5, unit = "cm"), 
               axis.text.x.top = element_text(hjust = 0, vjust = 1, angle = 90),
               axis.ticks.x.top = element_blank(),
               #axis.text.x.bottom = element_text(hjust = 1, vjust = 2, angle = 90),
               axis.text.x.bottom = element_blank(),
               axis.ticks.x.bottom = element_blank(),
               #axis.text.y.left = element_text(size = fontsize, color = "black", hjust = 1, margin = margin(b = 0, l = 0, t = 0, r = 0.15, unit = "cm")),
    ...
    if(!is.null(facetY) | !is.null(facetX)){
        if(is.null(facetX)){ facetX = "." }
        if(is.null(facetY)){ facetY = "." }
        p <- p +
             #geom_rect(aes(color = !!as.name(facetX)), xmin = -Inf, xmax = Inf, ymin = -Inf, ymax = Inf, size = 2, fill = NA) +
             #facet_grid(rows = vars(!!as.name(facetY)), cols = vars(!!as.name(facetX)),
             facet_grid(as.formula(paste(paste(facetY, collapse = " + "), "~", facetX)), 
                        scales = "free",
                        space = "free")
    }
...
plotSummaryBars <- function(dat, clrs, fillCol = "Group", xVar = "Group", yVar = "y", title = "Median Fraction",
    ...
    p <- ggplot(data = resTbl, aes_string(x = xVar, y = yVar)) +
         ...
         xlab(gsub(" ", "\n", title)) +
         scale_color_manual(values = allClrs, guide = "none") +
...
plotMedianBars <- function(dat, clrs, fillCol = "Group", xVar = "Group", yVar = "y", title = "Median Fraction",
    ...
    p <- ggplot(data = resTbl, aes_string(x = xVar, y = yVar)) +
         ...
         #geom_hline(yintercept = 0.5, size = 0.5, color = "darkgray") +
         geom_hline(yintercept = Inf, size = 0.5, color = "black") +
         geom_vline(xintercept = 0.25, size = 0.5, color = "black") +
         geom_vline(xintercept = xMax, size = 0.5, color = "black") +
         geom_text(aes(x = as.numeric(x), y = top - 0.05, label = signif), size = 5, color = "black") +
         xlab(gsub(" ", "\n", title)) +
         scale_color_manual(values = allClrs, guide = "none") +
         scale_fill_manual(name = "", values = allClrs, breaks = names(clrs)) +
         scale_y_discrete(expand = c(0,0)) +
         ...
         theme(legend.direction = "vertical",
               ...
               #plot.margin = margin(l = 0, r = -0.5, t = 3, b = 0.5, unit = "cm"))
               plot.margin = margin(l = -1.15, r = 0.5, t = 3, b = 0.5, unit = "cm"))
               #plot.margin = margin(l = -0.2, r = -1.1, t = 3, b = 0.5, unit = "cm"))
               #plot.margin = margin(l = -0.2, r = -0.83, t=3, b=0.5, unit = "cm"))
...
plotSeparateCohortVsIndividual <- function(dat, indivDat, cfg, statsFiles, sheet, comps, calcType, cellTypes, conds,
                                           ...
                                           facetY = NULL, facetOrder = NULL, popOrder = NULL, orderBy = NULL, stripeBG = TRUE,
                                           idOrder = NULL, xVar = "Sample_ID", xOrder = NULL, colorBarLimit = 5, 
                                           stripWidth = unit(2,"cm"), separateByDirection = FALSE, gridColor = "darkgray",
                                           facetX = NULL, hideConditionIDs = FALSE, indivHeatmap = TRUE){

    plotParams <- list("all" = list(hideXtitle = FALSE,
                                    hideXaxisTop = FALSE,
                                    hideXaxisBottom = FALSE, 
                                    keepLegend = TRUE),
                       "up" = list(hideXtitle = FALSE, 
                                   hideXaxisTop = FALSE,
                                   hideXaxisBottom = TRUE, 
                                   keepLegend = TRUE),
                       "down" = list(hideXtitle = TRUE, 
                                     hideXaxisTop = TRUE,
                                     hideXaxisBottom = TRUE, 
                                     keepLegend = FALSE),
                       "opposite" = list(hideXtitle = TRUE, 
                                         hideXaxisTop = TRUE,
                                         hideXaxisBottom = FALSE, 
                                         keepLegend = FALSE))
    strpBG  <- stripeBG
    ...
    pdfHeight <- 0
    rel_heights <- c()
    widths <- list(fractions = c(1.0, 2.0, 1.0, 0.4), #c(2, 1.0, 0.4, 1.0), #c(2, 0.4, 1.0), 
                   densities = c(1.0, 2.0, 1.0, 0.4)) #c(1.6, 0.8, 1, 0.4))
    ...
    for(type in sections){
        ...
        if(type == "all"){
            c2p <- dat %>%
                    ...
                    pull(`Cell State ID`)
            log_debug(paste("        [PLOT] conditions with", effectAbb, ">1 in at least one comparison:", 
                        length(c2p)))
        } else if(type == "up"){
            c2p <- dat %>%
                   filter((!!as.name(paste(comp1, "Overall", effectAbb)) > 1 &
        ...
        grps    <- unlist(comps)[grpIdxs] %>% unique %>% sort(decreasing = TRUE) ## UT non-CR CR
        ...
        #    log_error("Condition order in figure panels are different!!!")
        #    stop()
        #}
        ...
        if(indivHeatmap){
            ...
            ptPlot    <- plotComparisonHeatmap(indivDatT, "Sample_ID", yVar = "y", facetY = facetY,
                                               spacerColor = spacer, fill = "Value", colorBarLimit = colorBarLimit, 
                                               calcType = calcType, gridColor = gridColor, facetX = facetX, 
                                               hideXtitle = pp$hideXtitle, hideXaxisTop = pp$hideXaxisTop,
                                               hideXaxisBottom = pp$hideXaxisBottom, keepLegend = pp$keepLegend)
        
            plotList <- list(ptPlot, condTbl, statPlot, medians)        
        } else {
            plotList <- list(condTbl, statPlot, medians)
            widths <- widths[1:3]
        }

        log_debug("Creating plot grid")
        pg <- plot_grid(plotlist = plotList, align = 'hv', axis = 'bt', 
                        nrow = 1, rel_widths = widths[[calcType]])
        figs[[type]] <- pg
    ...
    pdfWidth <- ifelse(indivHeatmap, 11.5, 8.5)
