...
indexGeneralConditions <- function(condFile, sheets, arrangeAnnotation = NULL, firstID = 1){
    ...
    for(sheet in sheets){
        conds <- tryCatch({
                     read.xlsx(condFile, sheet, check.names=F) %>%
                     as_tibble %>%
...
indexConditions <- function(condFile, arrangeAnnotation){
    ...
    generalSheets <- c("fractions","densities")
    ...
    allConds <- bind_rows(general, spatial) %>%
                select_at(c("Cell State ID", "Subpopulation", "Population",
                          col_order[col_order %in% names(general)], "AnalysisType", "CalcType"))
...
getClassifier <- function(ct, cellTypes, facetCol){
    ctLvls <- c("Category", "Cell_type", "Subtype")
    celltypes <- cellTypes %>% select(all_of(ctLvls))
    sapply(ct, function(x){
        catType <- celltypes %>%
                   filter_at(vars(all_of(ctLvls)), any_vars(grepl(paste0("^",x,"$"), .))) %>%
...
getAllStatsHeatmapData <- function(questionsFile, resDir, calcType,
    ...
    if(length(qNums) == 1 && tolower(qNums) == "all"){
        qNums <- openread.xlsx(questionsFile) %>% as_tibble() %>% pull(QuestionNumber)
    }
    ...
    for(qNum in qNums){
        ...
        if(!is.null(filtSheet)){
            qFilt <- read.xlsx(statsFile, filtSheet, check.names=F) %>% as_tibble()
        }

        ## get complete list of data
        qDat <- NULL
        qDat <- read.xlsx(statsFile, resSheet, check.names=F) %>% as_tibble()
...
getCellTypeClass <- function(popStr, cellTypes, ctCol){
   classOrder <- c("Category", "Cell_type", "Subtype")
...
customOrder <- function(vals, cOrder){
    ordr <- sapply(c(sort(cOrder),NA), function(x){ ifelse(is.na(x), which(is.na(cOrder)), which(cOrder == x) )})
    sapply(vals, function(x){ ifelse(is.na(x), ordr[is.na(names(ordr))], ordr[x] )}) %>% unlist(use.names=F)
}
